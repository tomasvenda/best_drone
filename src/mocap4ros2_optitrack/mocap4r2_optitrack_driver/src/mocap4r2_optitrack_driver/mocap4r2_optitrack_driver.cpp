...
: ControlledLifecycleNode("mocap4r2_optitrack_driver_node")
{
  ...
  declare_parameter<std::vector<std::string>>("rigid_body_ids", std::vector<std::string>{});
...
OptitrackDriverNode::process_frame(sFrameOfMocapData * data)
{
  ...
  bool publish_rigid_bodies = mocap4r2_rigid_body_pub_->get_subscription_count() > 0;
  bool has_individual_pubs = !individual_pose_pubs_.empty();

  if (publish_rigid_bodies || has_individual_pubs) {
    ...
    for (int i = 0; i < data->nRigidBodies; i++) {
      ...
      auto it = individual_pose_pubs_.find(rb.rigid_body_name);
      if (it != individual_pose_pubs_.end()) {
...
OptitrackDriverNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  for (const auto & id : rigid_body_ids_) {
    std::string topic = "rigid_body_" + id + "/pose";
    individual_pose_pubs_[id] =
      create_publisher<geometry_msgs::msg::PoseStamped>(topic, rclcpp::QoS(1000));
    RCLCPP_INFO(get_logger(), "Created individual pose publisher on topic: %s", topic.c_str());
  }
...
OptitrackDriverNode::on_activate(const rclcpp_lifecycle::State & state)
{
  ...
  for (auto & [id, pub] : individual_pose_pubs_) {
    pub->on_activate();
  }
